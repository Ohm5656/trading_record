...
try {
  ...
  await page.getByLabel('Why this entry?').fill('Breakout retest held above the planned entry zone')
  ...
  assert(await page.locator('.chart-evidence').count() === 1, 'Trade plan evidence was not shown')
  assert((await page.locator('.chart-evidence').innerText()).includes('Breakout retest held'), 'Trade image caption was not shown')
  await page.locator('.chart-evidence').click()
  await page.locator('.lightbox-figure figcaption').waitFor()
  assert((await page.locator('.lightbox-figure figcaption').innerText()).includes('Breakout retest held'), 'Trade image caption was not shown in the lightbox')
  ...
  assert(await page.locator('.chart-evidence').count() === 1, 'Uploaded trade image was not saved')
  ...
  assert(!(await page.getByRole('button', { name: 'Long', exact: true }).isDisabled()), 'Trading should remain available after a losing trade')
  ...
  await page.locator('.calendar-grid').waitFor()
  const lossCell = page.locator('.day-cell.lose').filter({ hasText: '50' })
  assert(await lossCell.count() === 1, 'A losing day was not preserved as red after reload')
  ...
  await page.locator('.calendar-grid').waitFor()
  assert(await page.locator('.day-cell.lose').filter({ hasText: '50' }).count() === 1, 'Trade was not isolated to and restored for the logged-in user')
